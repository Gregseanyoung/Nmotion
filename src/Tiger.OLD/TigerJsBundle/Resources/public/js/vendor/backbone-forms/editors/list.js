...
define(['jquery', 'underscore', 'backbone', 'backbone-forms'], function($, _, Backbone) {
  ...
  editors.List = editors.Base.extend({
    ...
    render: function() {
      ...
      //Add existing items
      if (value.length) {
        var proceedItemsFn = function (itemValue) {
          self.addItem(itemValue);
        };
        if (value.each) {
          value.each(proceedItemsFn);
        } else {
          _.each(value, proceedItemsFn);
        }
      }

      //If no existing items create an empty one, unless the editor specifies otherwise
      else if (_(this.validators).indexOf('required') >= 0) {
    ...
    addItem: function(value, userInitiated) {
      ...
      if (self.schema.sealed && userInitiated) {
          return;
      }
    ...
    removeItem: function(item) {
      ...
      if (_(this.validators).indexOf('required') >= 0) {
