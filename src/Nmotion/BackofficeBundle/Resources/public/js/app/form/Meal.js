define([
    ...
    var MealForm = SymfonyForm.extend('MealForm', {
        ...
        events: {
            ...
            'click button.btn.bbf-add': function (event) {
                // to set currency sign in price input box
                var restaurantCurrency = this.model.get("menuCategory").get("restaurant").get("currency");
                if(typeof restaurantCurrency != "undefined" && typeof restaurantCurrency.sign != "undefined") {
                    this.$el.find("[name=priceIncludingTax]").prev(".add-on").text(restaurantCurrency.sign);
                }
            }
        },
