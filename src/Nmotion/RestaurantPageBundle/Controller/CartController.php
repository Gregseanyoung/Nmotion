...
use Nmotion\NmotionBundle\Entity\SolutionAdmin;
...
class CartController extends BaseRestController
{
	...
	if ($currentHost != 'ordery.nmotion.dk') {
        	$currentRestaurant = $this->getDoctrine()->getRepository('Nmotion\NmotionBundle\Entity\Restaurant')->findOneBy(['nmSiteUrl' => $currentHost]);
	} else {
		$currentRestaurant = $this->getDoctrine()->getRepository('Nmotion\NmotionBundle\Entity\Restaurant')->findOneBy(['nmSiteUrl' => 'ordery.nmotion.dk']);
	}
	#var_dump($currentRestaurant); die;
    ...
    {
	$currentRestaurant = $this->getRestaurantFromRequest($request);
	...
	if (count($userList) > 1) {
        ...
        } else if (count($userList) == 1 && (isset($userList[0]) && $userList[0] instanceof RestaurantGuest || $userList[0] instanceof SolutionAdmin)) {
            $user = $userList[0];
        } else {
            $user = (new RestaurantGuest)
                    ...
                    ->setRegistrationOrigin($this->getOrigin($currentRestaurant->getId()))
        ...
        $restaurant  = $currentRestaurant; //$this->getRepository('Restaurant')->find((int)$model['site_id']);
	...
	//echo '<pre>';
	//print_r($restaurant->getName());
	//print_r($request); die();
	//print_r($currentRestaurant->getName()); die;
