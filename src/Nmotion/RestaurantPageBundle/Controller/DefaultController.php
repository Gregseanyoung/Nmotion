...
class DefaultController extends BaseRestController
{
    private function getRestaurantFromRequest(Request $request)
    {
        ...
        $first = $this->getDoctrine()->getRepository('Nmotion\NmotionBundle\Entity\Restaurant')->findOneBy(['nmSiteUrl' => $currentHost, 'nmWebsite' => true]);
        if ($first === null) {
            $second = $this->getDoctrine()->getRepository('Nmotion\NmotionBundle\Entity\Restaurant')->findOneBy(['nmSiteUrl' => 'www.' . $currentHost, 'nmWebsite' => true]);
            if ($second === null) {
	...
	if ($currentName == 'tryg') { 
		header("HTTP/1.1 301 Moved Permanently");
		header('Location: http://camp.nmotion.dk');
	}
