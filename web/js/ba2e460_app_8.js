...
var app = angular.module('DashboardApp', ['ngRoute']).config(function($routeProvider, $interpolateProvider){
    $interpolateProvider.startSymbol('{[{').endSymbol('}]}');

    $routeProvider

        //.when('/', {})
        .when('/config/list', { templateUrl: '/template/dashboard/page/config/list.html', controller: 'ConfigListController' });/*
        .when('/restaurant/list', { templateUrl: '', controller: '' })
        .when('/restaurant/add', { templateUrl: '', controller: '' })
        .when('/user/list', { templateUrl: '', controller: '' })
        .when('/order/list', { templateUrl: '', controller: '' });*/
});
       ...
       controller: ('MainMenuController', ['$rootScope','$scope', function($rootScope, $scope) {
           $scope.menuItems = [
               { desc: 'Dashboard', icon: 'home', active: 'active', child: 'block', children: [
                   { desc: 'Config list', icon: '', link: '#/config/link'}
               ]},
